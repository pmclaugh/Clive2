...
if __name__ == "__main__":
    ...
    if args.scene == "empty":
        ...
        # load the teapots
        tris += load_obj(
            "../resources/teapot.obj", offset=np.array([0, 0, 2.5]), material=5
        )
        tris += load_obj(
            "../resources/teapot.obj", offset=np.array([0, 0, -2.5]), material=0
        )
        ...
        load_time = time.time()
        tris += load_ply(
            "../resources/dragon_vrip_res3.ply",
        ...
        load_time = time.time()
        tris += load_ply(
            "../resources/dragon_vrip_res2.ply",
            offset=np.array([0, -4, 0]),
            material=5,
            scale=50,
        )
        ...
        tris += load_ply(
            "../resources/dragon_vrip_res2.ply",
            offset=np.array([-3, -3.5, 0]),
            material=5,
            scale=40,
        )
        tris += load_ply(
            "../resources/dragon_vrip_res2.ply",
            offset=np.array([3, -3.5, -2]),
            material=0,
            scale=40,
        )
        print(f"done loading dragon in {time.time() - load_time}")
    ...
    summed_sample_weights = np.zeros(
        (c.pixel_height, c.pixel_width, 1), dtype=np.float32
    )
    ...
    light_surface_areas = dev.buffer(
        np.array([surface_area(t) for t in light_triangles], dtype=np.float32)
    )
    light_triangle_indices = np.array(
        [i for i, t in enumerate(triangles) if t["is_light"] == 1], dtype=np.int32
    )
    camera_triangle_indices = np.array(
        [i for i, t in enumerate(triangles) if t["is_camera"] == 1], dtype=np.int32
    )
    ...
    while f < args.total_frames:
        ...
        if args.total_frames > 1:
            ...
            summed_image = np.zeros(
                (c.pixel_height, c.pixel_width, 3), dtype=np.float32
            )
            summed_light_image = np.zeros(
                (c.pixel_height, c.pixel_width, 3), dtype=np.float32
            )
            summed_sample_counts = np.zeros(
                (c.pixel_height, c.pixel_width, 1), dtype=np.int32
            )
            summed_sample_weights = np.zeros(
                (c.pixel_height, c.pixel_width, 1), dtype=np.float32
            )
            ...
            for i in range(args.samples):
                ...
                if args.adaptive and i > 0:
                    bins, summed_bins, indices = get_adaptive_indices(
                        tone_map(summed_image / summed_sample_weights)
                    )
                else:
                ...
                trace_fn(
                    ...
                    out_camera_image,
                    out_camera_paths,
                    out_camera_debug_image,
                )

                # unidirectional outputs
                unidirectional_image = np.frombuffer(
                    out_camera_image, dtype=np.float32
                ).reshape(c.pixel_height, c.pixel_width, 4)[:, :, :3]
                image = unidirectional_image
                finalized_sample_counts = np.ones(
                    (c.pixel_height, c.pixel_width, 1), dtype=np.int32
                )
                finalized_sample_weights = np.ones(
                    (c.pixel_height, c.pixel_width, 1), dtype=np.float32
                )
                ...
                if not args.unidirectional:
                    ...
                    join_fn(
                        ...
                        final_out_samples,
                        out_light_indices,
                        out_light_path_indices,
                        out_light_ray_indices,
                        out_light_weights,
                        out_light_shade,
                    )

                    # retrieve outputs
                    bidirectional_image = np.frombuffer(
                        final_out_samples, dtype=np.float32
                    ).reshape(c.pixel_height, c.pixel_width, 4)[:, :, :3]

                    # finalize
                    adaptive_finalize_fn(
                        batch_size,
                        weight_aggregators,
                        camera_buffer,
                        finalized_samples,
                    ...
                    finalized_image = np.frombuffer(
                        finalized_samples, dtype=np.float32
                    ).reshape(c.pixel_height, c.pixel_width, 4)[:, :, :3]

                    # test: light indices
                    light_image_indices = np.frombuffer(
                        out_light_indices, dtype=np.int32
                    )
                    light_path_indices = np.frombuffer(
                        out_light_path_indices, dtype=np.int32
                    )
                    light_ray_indices = np.frombuffer(
                        out_light_ray_indices, dtype=np.int32
                    )
                    ...
                    bins = np.bincount(
                        ...
                        minlength=image.shape[0] * image.shape[1],
                    )
                    summed_bins = dev.buffer(
                    ...
                    if missed_count < batch_size * 8:
                        ...
                        light_image = np.frombuffer(
                            out_light_image, dtype=np.float32
                        ).reshape(c.pixel_height, c.pixel_width, 4)[:, :, :3]

                    finalized_sample_counts = np.frombuffer(
                        sample_counts, dtype=np.int32
                    ).reshape(c.pixel_height, c.pixel_width, 1)
                ...
                to_display = tone_map(
                    summed_image / summed_sample_weights, exposure=4.0
                )
        ...
        if args.total_frames == 1:
            cv2.imwrite(
                f'../output/{args.movie_name}/{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.png',
                to_display,
            )
        else:
        ...
        print(
            "most sampled pixel:",
            np.argmax(summed_sample_counts),
            "with",
            np.max(summed_sample_counts),
            "samples",
        )
