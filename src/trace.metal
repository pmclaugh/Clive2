...
float GGX_D(const thread float3 &m, const thread float3 &n, const thread float alpha) {
    ...
    float tan2 = (1.0 - cosTheta2) / cosTheta2;
    ...
    for (int t = 2; t < camera_path.length + 1; t++){
        for (int s = 0; s < light_path.length + 1; s++){
            ...
            if (s == 0) {camera_path.rays[t - 1].l_importance = light_path.rays[0].l_importance;}
                ...
                Ray a, b, c;
                a = get_ray(camera_path, light_path, t, s, s - 2); // light_path.rays[s - 2]
                b = get_ray(camera_path, light_path, t, s, s - 1); // light_path.rays[s - 1]
                c = get_ray(camera_path, light_path, t, s, s); // camera_path.rays[t - 1]
                ...
                if (t > 1) {
                    Ray d = get_ray(camera_path, light_path, t, s, s + 1); // camera_path.rays[t - 2]
            ...
            if (s != 0) {
                Ray a, b, c;
                a = get_ray(camera_path, light_path, t, s, s + 1); // camera_path.rays[t - 2]
                b = get_ray(camera_path, light_path, t, s, s); // camera_path.rays[t - 1]
                c = get_ray(camera_path, light_path, t, s, s - 1); // light_path.rays[s - 1]
